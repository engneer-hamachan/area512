...
#define CONSOLE_ROW_CAPACITY 20
#define CONSOLE_ROW_HEIGHT 12
#define CONSOLE_FONT_SIZE 12
#define CONSOLE_ROW_BYTES 128

static void *s_row_sprite;
static char s_row_text[CONSOLE_ROW_CAPACITY][CONSOLE_ROW_BYTES];
static int s_row_byte_count[CONSOLE_ROW_CAPACITY];
static int s_row_pixel_width[CONSOLE_ROW_CAPACITY];
...
static int
compute_visible_row_count(void) {
  int row_count = area512_gfx_height() / CONSOLE_ROW_HEIGHT;

  if (row_count > CONSOLE_ROW_CAPACITY)
    row_count = CONSOLE_ROW_CAPACITY;
...
advance_to_next_row(void) {
  if (s_cursor_row_index < compute_visible_row_count() - 1) {
    s_cursor_row_index++;
...
area512_console_reset(void) {
  for (int row_index = 0; row_index < CONSOLE_ROW_CAPACITY; row_index++)
    clear_row(row_index);
